...
let string_of_StateType = function
| EStateInvalid -> "EStateInvalid"
| EStateUnloaded -> "EStateUnloaded"
| EStateConnected -> "EStateConnected"
| EStateAttaching -> "EStateAttaching"
| EStateLaunching -> "EStateLaunching"
| EStateStopped -> "EStateStopped"
| EStateRunning -> "EStateRunning"
| EStateStepping -> "EStateStepping"
| EStateCrashed -> "EStateCrashed"
| EStateDetached -> "EStateDetached"
| EStateExited -> "EStateExited"
| EStateSuspended -> "EStateSuspended"
...
let string_of_EventType = function
| ENoEvent -> "ENoEvent"
| EBroadcastBitStateChanged -> "EBroadcastBitStateChanged"
| EBroadcastBitInterrupt -> "EBroadcastBitInterrupt"
| EBroadcastBitSTDOUT -> "EBroadcastBitSTDOUT"
| EBroadcastBitSTDERR -> "EBroadcastBitSTDERR"
| EBroadcastBitProfileData -> "EBroadcastBitProfileData"

type symbol_type =
| ESymbolTypeAny  (*         | ESymbolTypeInvalid *)
| ESymbolTypeAbsolute
| ESymbolTypeCode
| ESymbolTypeResolver
| ESymbolTypeData
...
let int_to_SymbolType = function
|  0 -> ESymbolTypeAny  (*         |  0 -> ESymbolTypeInvalid *)
|  1 -> ESymbolTypeAbsolute
|  2 -> ESymbolTypeCode
|  3 -> ESymbolTypeResolver
|  4 -> ESymbolTypeData
|  5 -> ESymbolTypeTrampoline
|  6 -> ESymbolTypeRuntime
|  7 -> ESymbolTypeException
|  8 -> ESymbolTypeSourceFile
|  9 -> ESymbolTypeHeaderFile
| 10 -> ESymbolTypeObjectFile
| 11 -> ESymbolTypeCommonBlock
| 12 -> ESymbolTypeBlock
| 13 -> ESymbolTypeLocal
| 14 -> ESymbolTypeParam
| 15 -> ESymbolTypeVariable
| 16 -> ESymbolTypeVariableType
| 17 -> ESymbolTypeLineEntry
| 18 -> ESymbolTypeLineHeader
| 19 -> ESymbolTypeScopeBegin
| 20 -> ESymbolTypeScopeEnd
| 21 -> ESymbolTypeAdditional
(* When symbols take more than one entry the extra entries get this type *)
| 22 -> ESymbolTypeCompiler
| 23 -> ESymbolTypeInstrumentation
| 24 -> ESymbolTypeUndefined
| 25 -> ESymbolTypeObjCClass
| 26 -> ESymbolTypeObjCMetaClass
| 27 -> ESymbolTypeObjCIVar
| 28 -> ESymbolTypeReExported
| _ -> ESymbolTypeAny


let int_of_SymbolType = function
| ESymbolTypeAny ->  0
...
let string_of_SymbolType = function
  | ESymbolTypeAny -> "ESymbolTypeAny"
  ...
  | ESymbolTypeUndefined -> "ESymbolTypeUndefined"
...
let int_to_AddressClass = function
| 0 -> EAddressClassInvalid
| 1 -> EAddressClassUnknown
| 2 -> EAddressClassCode
| 3 -> EAddressClassCodeAlternateISA
| 4 -> EAddressClassData
| 5 -> EAddressClassDebug
| 6 -> EAddressClassRuntime
| x -> Printf.kprintf failwith "int_to_AddressClass %d" x

let string_of_AddressClass = function
...
let int_to_EventType = function
  | 0 -> ENoEvent
  | 1 ->  EBroadcastBitStateChanged
  | 2 ->  EBroadcastBitInterrupt
  | 4 ->  EBroadcastBitSTDOUT
  | 8 ->  EBroadcastBitSTDERR
  | 16 ->  EBroadcastBitProfileData
  | x -> Printf.kprintf failwith "int_to_EventType %x" x
...
let eSymbolContextTarget     = 1 (*
 Set when \a target is requested from a query, or was located in query results
                                 *)
let eSymbolContextModule     = 1 lsl 1
        (* Set when \a module is requested from a query, or was located in query results *)
let eSymbolContextCompUnit   = 1 lsl 2
        (* Set when \a comp_unit is requested from a query, or was located in query results *)
let eSymbolContextFunction   = 1 lsl 3 (* Set when \a function is requested from a query, or was located in query results *)
let eSymbolContextBlock      = 1 lsl 4 (* Set when the deepest \a block is requested from a query, or was located in query results *)
let eSymbolContextLineEntry  = 1 lsl 5 (* Set when \a line_entry is requested from a query, or was located in query results *)
let eSymbolContextSymbol     = 1 lsl 6 (* Set when \a symbol is requested from a query, or was located in query results *)
let eSymbolContextEverything = (eSymbolContextSymbol lsl 1) - 1
